...
Each session also gets a scratch directory at `/tmp/hooop-session/<id>` that counts as inside its boundary, and the agent is told to use it for screenshots and throwaway scripts. Measured on a real session: that alone removes about 40% of the approval prompts, which were the agent writing to `/tmp` and reading its own output back.
