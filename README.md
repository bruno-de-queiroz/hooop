...
Each session also gets a scratch directory at `./tmp` inside its own working directory, and the agent is told to use it for screenshots and throwaway scripts, with `$TMPDIR`/`$TMP`/`$TEMP` pointed there so tools that never read the prompt land there anyway. Measured on a real session: that alone removes about 40% of the approval prompts, which were the agent writing to `/tmp` and reading its own output back. Inside the workdir it needs no policy of its own — that path was already routine for the model's file tools and already inside the Landlock allow-list for its shell. An earlier version lived at `/tmp/hooop-session/<id>` and cost three symlink-shaped bugs to defend, because `/tmp` is shared by every session in the container. Claude's own temp state follows the same env vars, so it lands there too instead of in the shared `/tmp`. What remains shared is `/tmp` itself, still readable and writable by every session's shell, which is an accepted risk rather than a closed one.
